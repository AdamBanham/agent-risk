...
class PolygonTerritory:
    ...
    def __init__(self, vertices: List[Point], territory_id: int = -1):
        ...
        :param territory_id: Unique ID for this territory (-1 if not
                            finalized)
        """
        self.vertices = [p if isinstance(p, Point) else Point(*p) for p in vertices]
    ...
    def divide(self) -> Tuple["PolygonTerritory", "PolygonTerritory"]:
        ...
        while (left is None or right is None) or (lprop < 0.3 or lprop > 0.7):
            ...
            walk = random_walk(lpivot, rpivot, num_steps=25, variation_strength=0.02)

            if choice < other:
                left = PolygonTerritory(self.vertices[choice + 1 : other] + walk[::-1])
                ...
                right = PolygonTerritory(self.vertices[other + 1 : choice] + walk)
            ...
            if attempts > 10:
                raise ValueError(
                    "Failed to divide polygon into two valid "
    ...
    def validate_vertices(self) -> Dict[str, any]:
        ...
        if len(self.vertices) < 3:
            errors.append(
                f"Insufficient vertices: {len(self.vertices)} " f"(minimum 3 required)"
            )

        # Check for finite coordinates
        for i, vertex in enumerate(self.vertices):
            if not isinstance(vertex, (Point)):
                errors.append(f"Vertex {i} is not a valid 2D coordinate: " f"{vertex}")
                continue

            x, y = vertex.x, vertex.y
            if not (isinstance(x, (int, float)) and isinstance(y, (int, float))):
                errors.append(
                    f"Vertex {i} contains non-numeric " f"coordinates: ({x}, {y})"
                )
                continue

            if not (math.isfinite(x) and math.isfinite(y)):
                errors.append(
                    f"Vertex {i} contains non-finite " f"coordinates: ({x}, {y})"
                )
                continue
        ...
        if bbox_area > 0 and area / bbox_area < 0.1:
            warnings.append(
                f"Low area efficiency: {area/bbox_area:.2%} "
        ...
        for i in range(len(self.vertices)):
            ...
            if current == next_vertex:
                errors.append(
                    f"Duplicate consecutive vertices at positions "
        ...
        for i in range(len(self.vertices)):
            ...
            cross_product = (p2.x - p1.x) * (p3.y - p1.y) - (p2.y - p1.y) * (
                p3.x - p1.x
            )

            if abs(cross_product) < 1e-6:  # Essentially collinear
                warnings.append(
                    f"Collinear vertices found at positions "
        ...
        if edge_lengths:
            ...
            very_short_edges = [
                i for i, length in enumerate(edge_lengths) if length < 1.0
            ]
            if very_short_edges:
                warnings.append(
                    f"Very short edges found at positions: " f"{very_short_edges}"
                )

            # Check for highly irregular edge distribution
            if len(edge_lengths) > 1:
                edge_variance = sum(
                    (length - properties["avg_edge_length"]) ** 2
                ...
                if properties["avg_edge_length"] > 0:
                    coefficient_of_variation = (
                        math.sqrt(edge_variance) / properties["avg_edge_length"]
                    )
                    properties["edge_length_cv"] = coefficient_of_variation

                    if coefficient_of_variation > 2.0:
                        warnings.append(
                            f"Highly irregular edge lengths (CV: {coefficient_of_variation:.2f})"
                        )
        ...
        if (
            min_x < reasonable_bounds[0]
            or min_y < reasonable_bounds[1]
            or max_x > reasonable_bounds[2]
            or max_y > reasonable_bounds[3]
        ):
            warnings.append(
                f"Vertices outside reasonable bounds: ({min_x}, {min_y}) to ({max_x}, {max_y})"
            )
    ...
    def _edges_intersect(self, p1: Point, p2: Point, p3: Point, p4: Point) -> bool:
...
def create_polygon_to_fill_space(
    dimensions: Tuple[int, int], portion: float = 0.75, attempt: int = 0
) -> PolygonTerritory:
    ...
    if not validation["is_valid"]:
        ...
        warnings.warn(
            f"Polygon validation failed on attempt {attempt}: {validation['errors']}",
            RuntimeWarning,
        )
...
class BoardGenerator:
    ...
    def generate_board(self, game_state: GameState) -> None:
        ...
        print(
            f"Created initial continent with {len(initial_polygon.vertices)} vertices, area: {initial_polygon.area():.2f}"
        )

        # Subdivide the polygon into the desired number of territories
        territory_polygons = self._subdivide_continent(
            initial_polygon, game_state.regions
        )
    ...
    def _subdivide_continent(
        self, initial_polygon: PolygonTerritory, target_regions: int
    ) -> List[PolygonTerritory]:
    ...
    def _create_territory_from_polygon(
        self, territory_id: int, poly_territory: PolygonTerritory
    ) -> Territory:
    ...
    def _calculate_adjacencies(self, territories: List[Territory]) -> None:
        ...
        print(
            f"Adjacency calculation complete: {total_adjacencies} total connections, "
            f"{avg_adjacencies:.1f} average per territory"
        )

    def _territories_share_boundary(
        self, territory_a: Territory, territory_b: Territory
    ) -> bool:
    ...
    def _edge_exists_in_polygon(
        ...
        for i in range(len(vertices)):
            ...
            if (v1 == start and v2 == end) or (v1 == end and v2 == start):
                return True

        return False

    def _assign_initial_territories(
        self, game_state: "GameState", territories: List[Territory]
    ) -> None:
        ...
        for player in players:
            ...
            print(
                f"Player {player.id} ({player.name}): {len(player_territories)} territories, {total_armies} armies (expected: {armies_per_player})"
            )

            if total_armies != armies_per_player:
                print(
                    f"ERROR: Player {player.id} has {total_armies} armies but should have {armies_per_player}"
                )


def generate_sample_board(
    game_state: GameState, width: int = 1000, height: int = 600
) -> None:
