...
class AttackPopup:
    ...
    def __init__(self, x: int, y: int, width: int = 400, height: int = 300):
        ...
        self.attack_button = Button(
            ...
            height=30,
            text="Attack!",
            action=UIAction.RESOLVE_ATTACK,
        )
    ...
    def show(self, attack_state: "AttackState") -> None:
        ...
        self.army_counter.value = min(
            attack_state.attacking_armies, attack_state.max_attacking_armies
        )
        ...
        elements.extend(
            [
                Button(
                    ...
                    y=self.army_counter.y,
                    width=25,
                    height=25,
                    text="+",
                    action=self.army_counter.increment_action,
                ),
                Button(
                    ...
                    height=25,
                    text="-",
                    action=self.army_counter.decrement_action,
                ),
            ]
        )
...
class TurnUI:
    ...
    def _create_ui_elements(self) -> None:
        ...
        self.undo_button = Button(
            ...
            height=30,
            text="Undo (U)",
            action=UIAction.UNDO_PLACEMENT,
        )

        self.advance_phase_button = Button(
            x=button_x + button_spacing,
            y=button_y,
            width=120,
            height=30,
            text="Next Phase",
            action=UIAction.ADVANCE_PHASE,
        )

        self.end_turn_button = Button(
            x=button_x + button_spacing * 2,
            y=button_y,
            width=100,
            height=30,
            text="End Turn",
            action=UIAction.END_TURN,
        )
        ...
        self.execute_movement_button = Button(
            ...
            height=30,
            text="Move Armies",
            action=UIAction.EXECUTE_MOVEMENT,
        )

        self.cancel_movement_button = Button(
            x=button_x + 380,
            y=button_y + 40,
            width=120,
            height=30,
            text="Canel Armies",
            action=UIAction.END_MOVEMENT,
        )
    ...
    def _update_ui_elements(self) -> None:
        ...
        self.phase_label.text = (
            f"Phase: {phase_names.get(self.current_turn.phase, 'Unknown')}"
        )
        ...
        if self.current_turn.phase == TurnPhase.PLACEMENT:
            ...
            self.advance_phase_button.enabled = (
                self.current_turn.reinforcements_remaining == 0
            )
        elif self.current_turn.phase == TurnPhase.ATTACKING:
    ...
    def _update_placement_ui(self) -> None:
        ...
        self.reinforcement_counter.text = (
            f"Reinforcements: {self.current_turn.reinforcements_remaining}"
        )
        ...
        self.movement_counter.visible = False
        self.execute_movement_button.visible = False
        self.cancel_movement_button.visible = False
    ...
    def _update_moving_ui(self) -> None:
        ...
        if self.current_turn.current_movement:
            ...
            self.movement_counter.max_value = (
                self.current_turn.current_movement.max_moving_armies
            )
            self.movement_counter.value = (
                self.current_turn.current_movement.moving_armies
            )
            self.execute_movement_button.visible = True
            self.execute_movement_button.enabled = (
                self.current_turn.current_movement.can_move()
            )
            self.cancel_movement_button.visible = True
        else:
            self.movement_counter.visible = False
            self.execute_movement_button.visible = False
            self.cancel_movement_button.visible = False
    ...
    def handle_click(self, pos: Tuple[int, int]) -> bool:
        ...
        if (
            self.movement_counter.visible
            and self.current_turn.phase == TurnPhase.MOVING
        ):
    ...
    def _handle_counter_click(self, pos: Tuple[int, int], counter: Counter) -> bool:
        ...
        """
        # Plus button
        plus_rect = pygame.Rect(counter.x + counter.width - 60, counter.y, 25, 25)
        ...
        # Minus button
        minus_rect = pygame.Rect(counter.x + counter.width - 30, counter.y, 25, 25)
    ...
    def handle_key_press(self, key: int) -> bool:
        ...
        if (
            key == pygame.K_u
            and self.current_turn.phase == TurnPhase.PLACEMENT
            and self.undo_button.enabled
        ):
        ...
        if pygame.K_1 <= key <= pygame.K_9:
            ...
            if (
                self.current_turn.phase == TurnPhase.ATTACKING
                and self.current_turn.current_attack
            ):
                ...
                and self.current_turn.current_movement
            ):
                # Set moving armies
