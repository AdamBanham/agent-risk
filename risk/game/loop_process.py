...
from risk.engine.risk import SimulationController, RiskRecordEngine
from risk.engine import SideEffectEngine
...
class GameLoop:
    ...
    def __init__(
        ...
        sim_delay: float = 1.0,
        sim_speed: int = 5,
        event_que: Queue = None,
        control_que: Queue = None,
    ) -> None:
        """
        ...
        self._tape = RiskRecordEngine()
        self.sim_controller = SimulationController(
            self.game_state, [SideEffectEngine(), self._tape]
        )
        add_player_engines(self.sim_controller, self.turn_manager)

        self.stack = self._tape.stack
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            self.renderer.add_renderer(
                StackRenderer(self.stack, reversed=True, picking=15)
            )
            ...
            def launch_ui_action(action: UIAction):
                self.control_que.put(UIActionEvent(action, dict()))
    ...
    def _handle_selection(self, input_event: InputEvent) -> None:
        ...
        self.control_que.put(TerritorySelectedEvent(territory_id))
    ...
    def _handle_toggle_pause(self, input_event) -> None:
        ...
        if self.paused:
            self.control_que.put(SystemInterruptEvent())
        else:
            self.control_que.put(SystemResumeEvent())
            self.control_que.put(SystemStepEvent())
    ...
    def _push_sim_sys_resume(self, input_event) -> None:
        ...
        """
        from ..state.event_stack import SystemResumeEvent, SystemStepEvent
    ...
    async def _simulation_loop(self) -> None:
        ...
        processed = 0
        self.control_que.put(True)
        while self.running:
            if not self.paused:
                step_time = time.time()

                for _ in range(self._sim_speed):
                    if not self.event_que.empty():
                        event = self.event_que.get()
                        info(f"collected event from simulation :: {event}")
                        self.sim_controller.force_processing_of(event)
                        processed += 1
                        self.sim_controller.stack.clear()

                if processed > 0:
                    elapsed = time.time() - step_time
                    self._caption = (
                        f"Agent Risk - Dynamic Board Simulation"
                        f" ({processed:0d} actions/render)"
                        f" (processed in {elapsed:.4f} seconds)"
                    )
                    info(
                        "Simulation step took {:.4f} seconds for "
                        "{processed:0d} actions".format(elapsed, processed=processed)
                    )
                processed = 0
