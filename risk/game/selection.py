...
class TerritorySelectionHandler:
    ...
    def __init__(
        self, game_state: GameState, turn_manager: Optional[TurnManager] = None
    ):
        """
        Initialize the selection handler. Sets up selection state tracking 
        for the given game state.
    ...
    def handle_territory_selected(self, input_event) -> None:
        ...
        print(
            f"Territory selection handler: Selected {territory.name} (ID: {territory_id})"
        )
    ...
    def set_action_callbacks(
        ...
        movement_callback: Optional[Callable] = None,
    ) -> None:
        """
    ...
    def _handle_turn_based_action_before_selection(self, territory: Territory) -> None:
        ...
        print(
            f"DEBUG: Current turn phase: {current_turn.phase.value}, player: {current_turn.player_id}"
        )
        print(
            f"DEBUG: Reinforcements remaining: {current_turn.reinforcements_remaining}"
        )
    ...
    def _handle_attack_click(self, territory: Territory, current_turn) -> None:
        ...
        """
        print(
            f"DEBUG: Attack click on {territory.name}, current phase: {current_turn.phase.value}"
        )
        print(
            f"DEBUG: Current selection state - primary: {self.primary_selection}, selected: {self.selected_territories}"
        )
        ...
        print(
            f"DEBUG: Primary territory: {primary_territory.name if primary_territory else 'None'}"
        )
        ...
        if not primary_territory or primary_territory.id == territory.id:
            ...
            if territory.owner == current_turn.player_id and territory.armies > 1:
                print(
                    f"Selected {territory.name} as attacker. Now click on adjacent enemy territory."
                )
                ...
                print(
                    f"DEBUG: Territory owner: {territory.owner}, player: {current_turn.player_id}, armies: {territory.armies}"
                )
            return

        print(
            f"DEBUG: Attempting attack from {primary_territory.name} (owner: {primary_territory.owner}) to {territory.name} (owner: {territory.owner})"
        )
        print(
            f"DEBUG: Primary territory adjacent: {[t.name for t in primary_territory.adjacent_territories]}"
        )
        print(f"DEBUG: Target territory ID: {territory.id}")

        # Check if this is a valid attack target
        if (
            territory.owner != current_turn.player_id  # Enemy territory
            and territory in primary_territory.adjacent_territories
        ):  # Adjacent

            # Start attack
            if current_turn.start_attack(primary_territory, territory):
                print(
                    f"Starting attack from {primary_territory.name} to {territory.name}"
                )
            ...
            if territory.owner == current_turn.player_id:
                ...
                print(
                    f"DEBUG: Available adjacent targets: {[self.game_state.territories[tid].name for tid in primary_territory.adjacent_territories if tid in self.game_state.territories]}"
                )
            else:
    ...
    def _handle_movement_click(self, territory: Territory, current_turn) -> None:
        ...
        if not primary_territory:
            ...
            if territory.owner == current_turn.player_id and territory.armies > 1:
                print(
                    f"Selected {territory.name} as movement source. Now click on adjacent owned territory."
                )
            else:
                print(
                    "Selected territory cannot move armies (need >1 army and ownership)"
                )
            return

        # Check if this is a valid movement target
        if (
            territory.owner == current_turn.player_id  # Owned territory
            and territory in primary_territory.adjacent_territories
        ):  # Adjacent
            ...
            if current_turn.start_movement(primary_territory, territory):
                print(
                    f"Starting movement from {primary_territory.name} to {territory.name}"
                )
