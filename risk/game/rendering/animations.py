...
class AnimationEngine(Engine):
    ...
    def process(self, game_state: GameState, element: Event):
        if isinstance(element, TransferArmies):
            ...
            self.manager.add_random_walk_animation(src, dst, 1.2)
            ...
            if amount < 0:
                self.manager.add_cross_animation(
                    center, 0.1, 1.2, colors.get("loss_troops")
                )
            elif amount > 0:
                self.manager.add_tick_animation(
                    center, 0.1, 1.2, colors.get("gain_troops")
                )
            ...
            self.manager.add_flash_animation(center, 1, colors.get("flash"))
            ...
            attacker = game_state.get_territory(element.context.attacking_territory_id)
            defender = game_state.get_territory(element.context.defending_territory_id)
            color = colors.get("player_colors")[
                attacker.owner % len(colors["player_colors"])
            ]

            self.manager.add_arrow_animation(
                attacker.center, defender.center, 1.2, color
            )
            ...
            if result.attacker_won():
                self.manager.add_tick_animation(
                    defender.center, 1.0, 1.2, colors.get("fight_success")
                )
            elif result.defender_won():
                self.manager.add_cross_animation(
                    defender.center, 1.0, 1.2, colors.get("fight_fail")
                )
