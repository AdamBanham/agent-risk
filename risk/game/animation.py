    ...
    def update(self, delta_time: float) -> None:
        ...
        if self.state == AnimationState.WAITING:
            ...
            if self.progress >= 1.0:
                self.state = (
                    AnimationState.AFTER if self.after > 0 else AnimationState.COMPLETED
                )
...
class ArrowAnimation(BaseAnimation):
    ...
    def render(self, surface: pygame.Surface) -> None:
        ...
        self._draw_arrow_head(
            surface, shadow_current, shadow_tail, shadow_color, self.head_size
        )
        ...
        pygame.draw.line(surface, enhanced_color, tail_pos, current_pos, self.width)

        # Draw main arrowhead
        self._draw_arrow_head(
            surface, current_pos, tail_pos, enhanced_color, self.head_size
        )
    ...
    def _get_arrow_tail_position(
        self, tail_length: float = 60.0
    ) -> Tuple[float, float]:
        """
...
class TickAnimation(BaseAnimation):
    ...
    def render(self, surface: pygame.Surface) -> None:
        ...
        pygame.draw.line(
            surface, enhanced_color, (x - size // 3, y), (x, y + size // 2), thickness
        )
...
class RandomWalkAnimation(BaseAnimation):
    ...
    def render(self, surface: pygame.Surface) -> None:
        ...
        shadow_points = [
            (x + shadow_offset, y + shadow_offset) for x, y in current_points
        ]
        ...
        pygame.draw.lines(
            tmp_surface, path_color, False, current_points, self.width  # Not closed
        )
        ...
        for i, fraction in enumerate(self.cumulative_fractions):
            if target_fraction >= fraction:
                ...
                if i > 0:
                    ...
                    segment_progress = (target_fraction - prev_fraction) / (
...
class FlashAnimation(BaseAnimation):
    ...
    def render(self, surface: pygame.Surface) -> None:
        ...
        pygame.draw.circle(
            temp_surface, flash_color, (current_size, current_size), current_size
        )
        ...
        pygame.draw.circle(
            temp_surface, core_color, (current_size, current_size), core_size
        )
...
class AnimationManager:
    ...
    def add_arrow_animation(
        ...
        animation = ArrowAnimation(
            start_pos=start_pos, end_pos=end_pos, duration=duration, color=color
        )

        self.animations.append(animation)
        return animation

    def add_cross_animation(
        self,
        position: Tuple[float, float],
        duration: float,
        delay: float,
        ...
        animation = CrossAnimation(
            position=position, duration=duration, color=color, delay=delay
        )

        self.animations.append(animation)
        return animation

    def add_tick_animation(
        self,
        position: Tuple[float, float],
        duration: float,
        delay: float,
    ...
    def add_random_walk_animation(
        ...
        animation = RandomWalkAnimation(
            start_pos=start_pos, end_pos=end_pos, duration=duration, delay=delay
        )
    ...
    def add_flash_animation(
        ...
        animation = FlashAnimation(
            position=position, duration=duration, color=color, size=size, delay=delay
        )
