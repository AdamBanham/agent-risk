...
class MovementState(BaseState):
    ...
    def get_possible_actions(self) -> List[Movement]:
        ...
        for network in self.network_map.networks:
            ...
            for node in self.network_map.safes_in_network(network):
                ...
                if self.map.get_node(node.id).value > 1:
                    for tgt in self.network_map.frontlines_in_network(network):
                        target = self.targets.get(tgt.id, 1)
            ...
            for node in self.network_map.frontlines_in_network(network):
                src_target = self.targets.get(node.id, 1)
                node = self.map.get_node(node.id)
                if self.map.get_node(node.id).value > src_target:
                    for tgt in self.network_map.frontlines_in_network(network):
                        tgt = self.map.get_node(tgt.id)
                        target = self.targets.get(tgt.id, 1)
                        if tgt.id != node.id and tgt.value < target:
    ...
    def take_action(self, action):
        ...
        amount = action.optimal_amount(
            self.targets.get(action.tgt.id, 1),
        )
    ...
    def get_reward(self):
        ...
        for network in self.network_map.networks:
            ...
            for tgt in self.network_map.frontlines_in_network(network):
                tgt = self.map.get_node(tgt.id)
                if tgt.value >= self.targets.get(tgt.id, 1):
                    rewards += 1
                else:
                    all_balanced = False
            if all_balanced:
...
def sum_of_adjacents(node: mapping.SafeNode, map, player) -> int:
    total = 0
...
class MovementPlanner(Planner):
    """
    A planner for moving troops aggresively using MCTS.
    """
    ...
    def construct_plan(self, state):
        ...
        # locate targets for each frontline in networks
        for network in network_map.networks:
            group = network_map.view(network)
            total_armies = sum(state.map.get_node(t.id).value for t in group.nodes)
            fronts = group.frontlines_in_network(network)
            moveable = total_armies - group.size
            weights = [sum_of_adjacents(t, state.map, self.player) for t in fronts]
            options = sorted(zip(fronts, weights), key=lambda x: x[1], reverse=True)[
                : random.randint(1, 3)
            ]

            for f, w in options:
                ideal_troops = max(1, int(moveable * w))
                targets[f.id] = ideal_troops

        debug(f"Determined movement targets: {targets}")
