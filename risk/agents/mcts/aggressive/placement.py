...
class Placement(BaseAction):
    ...
    def __init__(self, territory: int, placing: int):
        super().__init__()
        self.territory = territory
        self.placing = placing

    def to_step(self):
        return TroopPlacementStep(self.territory, self.placing)
...
class PlacementState(BaseState):
    ...
    def get_possible_actions(self) -> List[Placement]:
        ...
        if self.placements_left > 0:
            for terr in self.fronts:
                actions.append(Placement(terr.id, self.placements_left))
        return actions

    def take_action(self, action: Placement) -> "PlacementState":
        new_map = self.map.clone()
        new_map.get_node(action.territory).value += self.placements_left
        return PlacementState(
            0, new_map, self.player, self.frontlines
        )
    ...
    def get_reward(self):
        armies = sorted(
            [sum_of_adjacents(t, self.map, self.player) for t in self.fronts],
            reverse=True,
        )
