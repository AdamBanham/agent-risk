...
class Attack(BaseAction):
    ...
    def optimal_troops(self):
        if self.safe():
            return max(self.def_troops + 5, self.def_troops * 3)
        else:
            return self.atk_troops

    def to_step(self):
        return AttackStep(self.attacker, self.defender, self.optimal_troops())
...
class AttackState(BaseState):
    ...
    def get_possible_actions(self) -> List[Attack]:
        actions = []
        if self.attacks < self.max:
            for terr in self.frontlines:
                terr = self.map.get_node(terr.id)
                if terr.owner != self.player or terr.value <= 1:
                    continue
                for adj in self.map.get_adjacent_nodes(terr.id):
                    if adj.owner != self.player:
                        action = Attack(terr.id, adj.id, terr.value - 1, adj.value)
                        if action.safe():
    ...
    def take_action(self, action: Attack) -> "AttackState":
        ...
        new_map.get_node(action.attacker).value -= action.optimal_troops()
        if action.safe():
            new_map.get_node(action.defender).owner = self.player
            new_map.get_node(action.defender).value = max(1, action.optimal_troops() // 2)
        fronts = set(self.frontlines).difference(set([new_map.get_node(action.attacker)]))
        return AttackState(
            new_map, fronts, self.player, self.max, self.attacks + 1
        )

    def is_terminal(self) -> bool:
        return (
            self.attacks >= self.max
            or len([f for f in self.frontlines if self.map.get_node(f.id).value > 1])
            == 0
            or self._terminal
        )
...
class AttackPlanner(Planner):
    ...
    def construct_plan(self, state: GameState) -> AttackPlan:
        ...
        mcts_state = AttackState(
            state.map.clone(),
            set(state.map.get_node(f.id) for f in safe_map.frontline_nodes),
            self.player,
            self.max_attacks,
        )
