    ...
    def process(self, game_state: GameState, element: Union[Event, Level]) -> None:
        ...
        if isinstance(element, GameEvent):
            ...
            if winner is None:
                ret.append(PlayingEvent())
                ret.append(
                    AgentTurnPhase(game_state.total_turns, game_state.current_player_id)
                )
        return ret
...
class RiskTurnEngine(Engine):
    ...
    def process(self, game_state: GameState, element: Union[Event, Level]) -> None:
        ...
        if isinstance(element, PlacementPhase):
            return [
                PlacementPhaseEndEvent(
                    game_state.total_turns, game_state.current_player_id
                ),
            ]
            ...
            return [
                AgentTurnEndEvent(game_state.total_turns, game_state.current_player_id),
                MovementPhaseEndEvent(
                    game_state.total_turns, game_state.current_player_id
                ),
            ]
...
from ..state.event_stack.events.turns import TroopPlacementEvent

from ..state.event_stack import AdjustArmies, RejectTroopPlacement
...
class RiskPlacementEngine(Engine):
    ...
    allowed_elements = [PlacementPhaseEndEvent, TroopPlacementEvent]
    ...
    def _handle_placement(
        self, game_state: GameState, element: TroopPlacementEvent
    ) -> List[Event]:
        """
        ...
        if game_state.placements_left < 1 or num_troops > game_state.placements_left:
            return [
                RejectTroopPlacement(
                    game_state.total_turns,
                    player,
        ...
        return [AdjustArmies(territory, num_troops)]
...
from ..state.event_stack import CaptureTerritory, CasualitiesOnTerritory, TransferArmies
...
class RiskAttackEngine(Engine):
    ...
    allowed_elements = [AttackOnTerritoryEvent, FightEvent, ResolveFightEvent]
    ...
    def process(self, game_state: GameState, element: Union[Event, Level]) -> None:
        if isinstance(element, AttackOnTerritoryEvent):
            return self._handle_attack_request(game_state, element)
        elif isinstance(element, FightEvent):
            return self._handle_fight(game_state, element)
        elif isinstance(element, ResolveFightEvent):
            return self._handle_resolve_fight(game_state, element)
        return super().process(game_state, element)

    def _handle_attack_request(
        self, game_state: GameState, element: AttackOnTerritoryEvent
    ) -> List[Event]:

        attacker = game_state.get_territory(element.context.from_territory)
        defender = game_state.get_territory(element.context.to_territory)
        ...
        if (
            element.context.attacking_troops >= attacker.armies
    ...
    def _handle_fight(self, game_state: GameState, element: FightEvent) -> List[Event]:
    ...
    def _handle_resolve_fight(
        self, game_state: GameState, element: ResolveFightEvent
    ) -> List[Event]:
        ...
        if atk_cas > 0:
            ret.append(CasualitiesOnTerritory(atk_id, atk_cas))

        # report on attacker losses
        if def_cas > 0:
            ret.append(CasualitiesOnTerritory(def_id, def_cas))
...
from ..state.event_stack import RejectTransfer
...
class RiskMovementEngine(Engine):
    ...
    allowed_elements = [MovementPhaseEndEvent, MovementOfTroopsEvent]
    ...
    def process(self, game_state: GameState, element: Union[Event, Level]) -> None:
        if isinstance(element, MovementPhaseEndEvent):
            ...
            return self._handle_movement(game_state, element)

    def _handle_movement(
        self, game_state: GameState, element: MovementOfTroopsEvent
    ) -> List[Event]:

        from_territory = game_state.get_territory(element.context.from_territory)
        to_territory = game_state.get_territory(element.context.to_territory)
        ...
        if from_territory.owner != player or to_territory.owner != player:
            return [
                RejectTransfer(
                    game_state.total_turns,
                    player,
        ...
        if num_troops < 1 or num_troops > from_territory.armies:
            return [
                RejectTransfer(
                    game_state.total_turns,
                    player,
